...
class CampaignCharacterStateStore:
    ...
    def _read_position(
        ...
        for source in (
            self._read_legacy_map(campaign.positions),
            self._read_legacy_map(campaign.state.positions_parent),
            self._read_legacy_map(campaign.state.positions),
        ):
    ...
    def _read_hp(self, campaign: Campaign, character_id: str, actor_hp: object) -> int:
        value = self._read_legacy_map(campaign.hp).get(character_id)
    ...
    def _read_character_state(
        ...
        value = self._read_legacy_map(campaign.character_states).get(character_id)
        if isinstance(value, str):
            return value
    ...
    def _read_legacy_map(self, value: object) -> Dict[str, Any]:
