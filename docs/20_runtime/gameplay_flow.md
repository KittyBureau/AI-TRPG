...
- `midnight_archive_world` now uses this as a real playable path on `janitor_npc`: disabling the janitor can expose a `routing_slip` source that still feeds into the existing service-route bypass
