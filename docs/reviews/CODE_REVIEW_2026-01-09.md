...
- 上下文构建支持 system prompt + 注入块（角色卡/规则/世界状态）+ 历史消息拼接的 V0 方式。
...
- `context_strategy` 当前仅支持 `full_context`，历史对话完整拼接；`compact_context` 标记为待实现。
- system prompt 由文件加载（`backend/prompts/system/context_full.txt`），并可按 `mode` 切换。
- 注入块顺序与路径由 config.json 控制，支持文件或目录读取，未配置项自动跳过。
...
- `codes/backend/storage/config_template.json` 提供上下文策略、注入优先级与路由模板，便于实例化。
...
## 6. 当前刻意未实现或被延后的能力
- `compact_context` 与关键信息替代、自动摘要触发、固定信息 pin、token 预算等均在 TODO 中，尚未落地。
...
- 依据 `docs/TODO_CONTEXT.md` 推进 compact_context、关键事实抽取、自动摘要与回归验证，补齐上下文压缩链路。
