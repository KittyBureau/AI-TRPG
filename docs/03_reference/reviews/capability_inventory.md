...
- Startup command (docs): `uvicorn backend.api.main:app --reload` (`README.md`, `docs/02_guides/testing/api_test_guide.md`).
- Default port: not set in code; docs reference `http://127.0.0.1:8000` (`docs/02_guides/testing/map_generate_manual_test.md`).
...
- `campaign.json` fields defined by `backend/domain/models.py:Campaign` and detailed in `docs/01_specs/storage_layout.md`.
...
- Conflict types and behavior also described in `docs/01_specs/conflict_and_retry.md`.
...
- Tool specs documented in `docs/01_specs/tools.md`.
...
- Storage location in campaign: `campaign.json.settings_snapshot` + `settings_revision` (see `docs/01_specs/storage_layout.md`).
...
- Manual test guides:
- `docs/02_guides/testing/api_test_guide.md` — step-by-step API flows.
- `docs/02_guides/testing/map_generate_manual_test.md` — manual map_generate regression/smoke guide (mentions `MAP_TEST_STRICT`, `MAP_TEST_MAX_ATTEMPTS`).

## J. Unknowns / Gaps (evidence-based)
- Default port not set in code; only mentioned in docs (`docs/02_guides/testing/map_generate_manual_test.md`).
...
- Some docs reference `FakeLLM` flow (e.g., `docs/01_specs/architecture.md`), but runtime code uses `LLMClient` (`backend/infra/llm_client.py`); this mismatch is not clarified in code.
