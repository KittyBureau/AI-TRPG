...
- `GET /api/v1/campaign/get` returns the authoritative selected/actors/map/status snapshot for a valid campaign; `actors` exposes actor runtime snapshot fields needed by Play refresh, `map.areas` exposes current reachability state, `status` carries lifecycle + milestone data from the same campaign snapshot, missing campaigns return `404`, and invalid persisted campaign payloads return `500` with a stable error detail.
